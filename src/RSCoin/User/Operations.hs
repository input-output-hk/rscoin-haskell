...
import           Data.List              (elemIndex, foldl1', genericIndex, genericLength,
                                         groupBy, nub, sortOn)
...
import           RSCoin.User.Error      (UserError (..), UserLogicError,
                                         isWalletSyncError)
import           RSCoin.User.Logic      (SignatureBundle, getExtraSignatures, joinBundles,
                                         validateTransaction)
...
submitTransactionMapper st outputCoin outputAddr address requestedCoins = do
    ...
    let -- Pairs of chosen addrids and change for each color
        ...
        changesValues
            :: [C.Coin]
        changesValues = filter C.isPositiveCoin $ map snd chosenMap

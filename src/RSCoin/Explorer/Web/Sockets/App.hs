...
mkClientContext = ClientContext False Nothing Nothing
...
notifyAboutAddressUpdate addr = do
    ...
    let sessIds =
            fromMaybe S.empty $ connectionsState ^. csAddressSubscribers .
            at addr
        idToConn i = connectionsState ^? csClients . at i . _Just . ccConnection
